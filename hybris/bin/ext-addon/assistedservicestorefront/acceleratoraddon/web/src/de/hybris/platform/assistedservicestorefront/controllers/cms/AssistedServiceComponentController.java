...
public class AssistedServiceComponentController extends AbstractController
{
	...
	protected String getCustomerJSON(final CustomerModel customerModel) throws UnsupportedEncodingException
	{
		//		final String cardNumber = customerModel.getDefaultPaymentInfo() instanceof CreditCardPaymentInfoModel
		//				? ((CreditCardPaymentInfoModel) customerModel.getDefaultPaymentInfo()).getNumber() : null;
		...
		//		final String formattedCreationDate = customerModel.getCreationtime() != null
		//				? new SimpleDateFormat("dd/MM/YYYY").format(customerModel.getCreationtime()) : "--/--/----";
		B2BCustomerModel b2BcustomerModel = (B2BCustomerModel) customerModel;
		return String.format("{email:'%s',value:'%s',erpCode:'%s',unit:'%s'",
				XSSEncoder.encodeJavaScript(b2BcustomerModel.getUid()), XSSEncoder.encodeJavaScript(b2BcustomerModel.getName()),
				XSSEncoder.encodeJavaScript(b2BcustomerModel.getDefaultB2BUnit().getErpCode()),
				XSSEncoder.encodeJavaScript(b2BcustomerModel.getDefaultB2BUnit().getName()));
	}
