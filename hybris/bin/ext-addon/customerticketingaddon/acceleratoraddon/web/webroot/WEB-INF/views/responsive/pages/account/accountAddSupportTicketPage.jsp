...
<c:set var="productCode" >${param.c}</c:set>
<c:set var="productName" >${param.n}</c:set>
<c:choose>
	<c:when test="${empty productCode or empty productName}">
	                        ...
	                        <c:if test="${not empty categories}">
	                            <div class="form-group">
	                            	<c:forEach var="category" items="${categories}">
	                                   	<c:if test="${'SUPPORT' eq category}">
	                                   		<c:set var="defaultCategory" >${category}</c:set>
	                                   	</c:if>
	                                 	<c:set var="argument" >
	                                   		<spring:message code="text.account.supporttickets.createTicket.ticketCategory.${category}"/>
	                                   	</c:set>
	                                </c:forEach>
	                                <label class="control-label" for="text.account.supporttickets.createTicket.defaultTicketCategory"> 
	                                	<spring:theme code="text.account.supporttickets.createTicket.defaultTicketCategory" arguments="${argument}"/>
	                                </label>
									<input id="ticketCategory" name="ticketCategory" value="${defaultCategory}" hidden="true">   
	                            </div>	                        
	                        </c:if>
	...
	<c:otherwise>
		<jsp:include page="accountQuoteRequestPage.jsp" />  
	</c:otherwise>
</c:choose>