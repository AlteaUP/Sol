                ...
                <div class="item-group">
                    <ycommerce:testId code="orderDetail_overviewOrderStatus_label">
                        ...
                        <c:if test="${not empty orderData.statusDisplay}">
                            <span class="item-value"><spring:theme code="text.account.order.status.display.${fn:escapeXml(order.status)}"/></span>
                        </c:if>
                    </ycommerce:testId>
                </div>
            </div>
            <div class="col-sm-4 item-wrapper">
                <div class="item-group">
                    <ycommerce:testId code="orderDetail_overviewStatusDate_label">
                        ...
                        <span class="item-value"><fmt:formatDate value="${order.created}" dateStyle="medium" timeStyle="short" type="both" pattern="dd/MM/yy"/></span>
                    </ycommerce:testId>
                </div>
                <div class="item-group">
                ...
                <div class="item-group">
                    <c:if test="${orderData.paymentType.code=='ACCOUNT'}">
                        <ycommerce:testId code="orderDetail_overviewParentBusinessUnit_label">
                            ...
                            <span class="item-value">${fn:escapeXml(order.erpCode)} - ${fn:escapeXml(order.b2bUnitName)}</span>
                        </ycommerce:testId>
                    </c:if>
                </div>
                <c:if test="${displayQuoteInfo and not empty orderData.quoteExpirationDate}">
                    <div class="item-group">
                        <ycommerce:testId code="orderDetail_overviewQuoteExpirationDate_label">
                            ...
                            <span class="item-value"><fmt:formatDate value="${order.quoteExpirationDate}" dateStyle="medium" timeStyle="short" type="both" pattern="dd/MM/yy"/></span>
                        </ycommerce:testId>
                    </div>
                </c:if>
                <c:if test="${!orderData.refill}">
	                <div class="item-group">
