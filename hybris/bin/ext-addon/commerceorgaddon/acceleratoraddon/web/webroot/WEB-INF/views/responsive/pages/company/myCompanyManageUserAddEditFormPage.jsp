...
<%@ taglib prefix="customFormElement"
	tagdir="/WEB-INF/tags/addons/commerceorgaddon/responsive/customFormElement"%>
...
<c:if test="${empty saveUrl}">
	<c:choose>
		<c:when test="${not empty b2BCustomerForm.uid}">
			<spring:url
				value="/my-company/organization-management/manage-users/edit"
				var="saveUrl" htmlEscape="false">
		...
		<div class="account-section-content">
			<form:form action="${saveUrl}" method="post"
				...
				<div class="row">
					...
					<div class="col-xs-12 col-sm-12">
						<formElement:formInputBox idKey="user.email" labelKey="user.email"
							path="email" inputCSS="text" mandatory="true" />
					</div>
					<%--                     <c:set var="itemLabels">erpCode,name</c:set> --%>
<%-- 					<c:set var="itemLabels" value="${[erpCode, name]}" scope="application" /> --%>
<%-- 					<c:set var="itemLabels" value="${fn:split('erpCode, name', ',')}" scope="application" /> --%>
					
					<div class="col-xs-12 col-sm-12">
						<formElement:formSelectBox idKey="text.company.user.unit.title"
							skipBlank="false" labelKey="text.company.user.unit.title"
							path="parentB2BUnit" selectCSSClass="form-control"
							mandatory="true" items="${b2bUnitsMap}"
