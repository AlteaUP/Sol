...
public class MyCompanyPageController extends AbstractSearchPageController
{
	...
	protected static final String MANAGE_USERS_URL = "/my-company/organization-management/manage-users";
	...
	protected static final String REDIRECT_TO_USERS_LIST = REDIRECT_PREFIX + MANAGE_USERS_URL;
	...
	@Resource(name = "solgroupB2BUserFacade")
	protected B2BUserFacade b2bUserFacade;

	@Resource(name = "b2bUnitFacade")
	protected SolgroupB2BUnitFacade b2bUnitFacade;
	...
	@ModelAttribute("b2bUnitsMap")
	public List<SelectOption> getB2BUnitsMap()
	{
		return populateSelectBoxForStringFromMap(b2bUnitFacade.getAllActiveUnitsOfOrganizationWithErpCodeAndName());
	}
	...
	protected String unitDetails(final String unit, final Model model) throws CMSItemNotFoundException
	{
		...
		if (!Strings.isBlank(unitData.getErpCode()))
		{
			final List<Breadcrumb> breadcrumbs = myCompanyBreadcrumbBuilder
					.createManageUnitsDetailsBreadcrumbs(unitData.getErpCode() + " - " + unitData.getName());
	...
	protected String manageUnitDetail(final String unit) throws UnknownIdentifierException
	{
		try 
		{
		if (!Strings.isBlank(unit))
			{
				...
				return unitData.getErpCode() + " - " + unitData.getName();
			}
			return unit;
		}
	...
	protected List<SelectOption> populateSelectBoxForStringFromMap(final Map<String, String> mapOfStrings)
	{
		final List<SelectOption> selectOptions = new ArrayList<SelectOption>();

		for (String key : mapOfStrings.keySet())
		{
			selectOptions.add(new SelectOption(key, mapOfStrings.get(key)));
		}
