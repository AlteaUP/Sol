...
import com.solgroup.acceleratorstorefrontcommons.util.SolgroupAddressDataUtil;
...
public class BusinessUnitManagementPageController extends MyCompanyPageController
{
	...
	@Resource(name = "solgroupAddressDataUtil")
	private SolgroupAddressDataUtil solGroupAddressDataUtil;
			...
			throws CMSItemNotFoundException
	{
		...
		else
		{
			...
			unitData = (B2BUnitData) b2bUnitFacade.getParentUnit();
	...
	public String addAddress(@RequestParam("unit") final String unit, final Model model) throws CMSItemNotFoundException
	{
		...
		storeCmsPageInModel(model, getContentPageForLabelOrId(MANAGE_UNITS_CMS_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(MANAGE_UNITS_CMS_PAGE));
		final List<Breadcrumb> breadcrumbs = myCompanyBreadcrumbBuilder.createManageUnitsDetailsLabeledBreadcrumbs(unit,
				manageUnitDetail(unit));
		breadcrumbs.add(new Breadcrumb(
			...
			throws CMSItemNotFoundException
	{
		...
		final AddressData newAddress = solGroupAddressDataUtil.convertToAddressData(addressForm);
			...
			final Model model, final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		...
		try
		{
			 unitName = manageUnitDetail(unit);
		}
		...
		if (unit != null)
		{
			for (final AddressData addressData : unitData.getAddresses())
			{
				if (addressData.getId() != null && addressData.getId().equals(addressId))
				{
					...
					solGroupAddressDataUtil.convertBasic(addressData, addressForm);
					break;
				}
			}
		}
		...
		storeCmsPageInModel(model, getContentPageForLabelOrId(MANAGE_UNITS_CMS_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(MANAGE_UNITS_CMS_PAGE));
		final List<Breadcrumb> breadcrumbs = myCompanyBreadcrumbBuilder.createManageUnitsDetailsLabeledBreadcrumbs(unit, unitName);
		breadcrumbs
				.add(new Breadcrumb(
		...
		{ unitName }, "Edit Address for {0} Business Unit ", getI18nService().getCurrentLocale()), null));
