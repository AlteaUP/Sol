...
<c:if test="${not empty facetData.values}">
	<ycommerce:testId code="facetNav_title_${facetData.name}">
		<div class="facet js-facet">
			<div
				class="facet__name facet-name-filter js-facet-name div-facet-name-filter"
				role="button" data-toggle="collapse" data-parent="#accordion"
				href=".collapse-${facetData.code}" aria-expanded="true"
				aria-controls="${facetData.code}">
				<spring:theme code="search.nav.facetTitle"
					arguments="${facetData.name}" />
				<i class="glyphicon glyphicon-menu-right" style="float: right"
					aria-hidden="true"></i>
			</div>


			<div
				class="panel-collapse collapse collapse-${facetData.code} facet__values js-facet-values js-facet-form">
						...
						<c:forEach items="${facetData.topValues}" var="facetValue">
							<li><c:if test="${facetData.multiSelect}">
									...
									<span class="facet__text"> <a
										href="${facetValueQueryUrl}">${fn:escapeXml(facetValue.name)}</a>
