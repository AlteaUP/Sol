                        ...
                        <div class="col-xs-12 col-sm-4 col-md-4">
                            <div class="form-group">
                                ...
                                <form id="sortForm${top ? '1' : '2'}" name="sortForm${top ? '1' : '2'}" method="get"
                                    ...
                                    <!-- CR Wam Integration, documentType is used in accountOrderHistoryPage.jsp -->
                                    <c:if test="${not empty selectedDocumentType}">
                                        <input type="hidden" name="documentType" value="${selectedDocumentType}"/>
                                    </c:if>
                                    <!-- CR Wam Integration, startDate is used in accountOrderHistoryPage.jsp -->
                                    <c:if test="${not empty selectedSearchFromDateString}">
                                        <input type="hidden" name="searchFromDate" value="${selectedSearchFromDateString}"/>
                                    </c:if>
                                    <!-- CR Wam Integration, endDate is used in accountOrderHistoryPage.jsp -->
                                    <c:if test="${not empty selectedSearchToDateString}">
                                        <input type="hidden" name="searchToDate" value="${selectedSearchToDateString}"/>
                                    </c:if>
