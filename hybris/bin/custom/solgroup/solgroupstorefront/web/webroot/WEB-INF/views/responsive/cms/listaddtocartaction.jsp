...
<c:if test="${not product.multidimensional and (not empty product.price)}">
	...
	<form:form id="addToCartForm${fn:escapeXml(product.code)}" action="${addToCartUrl}" method="post" class="add_to_cart_form">
        ...
        <ycommerce:testId code="addToCartButton">
            ...
            <c:choose>
                ...
                <%-- 
                Show no addToCart button if product is outOfStock 
                <c:otherwise>
                	<button type="submit" class="btn btn-primary btn-block glyphicon glyphicon-shopping-cart" disabled="true"></button>
                </c:otherwise>
                --%>
            </c:choose>
        </ycommerce:testId>
    ...
    <form:form id="configureForm${fn:escapeXml(product.code)}" action="${configureProductUrl}" method="get" class="configure_form">
        <c:if test="${product.configurable}">
            <c:choose>
                <c:when test="${product.stockable and product.stock.stockLevelStatus.code eq 'outOfStock' }">
