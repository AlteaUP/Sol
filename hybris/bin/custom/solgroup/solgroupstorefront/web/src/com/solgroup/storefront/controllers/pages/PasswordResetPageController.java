...
public class PasswordResetPageController extends AbstractPageController
{
			...
			throws CMSItemNotFoundException
	{
		...
		else
		{
			try
			{
				customerFacade.forgottenPassword_searchByEmail(form.getEmail());
			}
			...
			final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		...
		if (!StringUtils.isBlank(form.getToken()))
		{
			...
			catch (final TokenInvalidatedException e)
			{
				...
				GlobalMessages.addFlashMessage(redirectModel, GlobalMessages.ERROR_MESSAGES_HOLDER, "updatePwd.token.expiredOrInvalid");
				//prepareErrorMessage(model, UPDATE_PWD_CMS_PAGE);
			} 
				...
				bindingResult.rejectValue("pwd", "updatePwd.notValidReason.regex");
				prepareErrorMessage(model, UPDATE_PWD_CMS_PAGE);
				return ControllerConstants.Views.Pages.Password.PasswordResetChangePage;
				...
				bindingResult.rejectValue("pwd", "updatePwd.notValidReason.generic");
				prepareErrorMessage(model, UPDATE_PWD_CMS_PAGE);
				return ControllerConstants.Views.Pages.Password.PasswordResetChangePage;
			}
