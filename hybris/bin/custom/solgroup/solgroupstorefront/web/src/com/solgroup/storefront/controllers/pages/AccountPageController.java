...
import com.solgroup.storefront.forms.SolGroupOrderHistoryFiltersForm;
...
public class AccountPageController extends AbstractSearchPageController {
    ...
    @Resource(name = "solGroupOrderFacade")
    private SolGroupOrderFacade orderFacade;
    ...
    @Resource(name = "solGroupOrderGridFormFacade")
    private SolGroupOrderGridFormFacade orderGridFormFacade;

    @Resource(name = "addressDataUtil")
    private AddressDataUtil addressDataUtil;
    
    @Resource(name = "consignmentFacade")
    private SolGroupConsignmentFacade consignmentFacade;
    ...
    @RequireHardLogIn
    public String orders(@ModelAttribute("filter") final SolGroupOrderHistoryFiltersForm filtersForm,
        ...
        if (StringUtils.isNotEmpty(filtersForm.getOrderStatus()))
            orderStatus = OrderStatus.valueOf(filtersForm.getOrderStatus());
        final PageableData pageableData = createPageableData(page, 5, sortCode, showMode);
        final SearchPageData<OrderHistoryData> searchPageData = orderFacade.getPagedOrderHistoryForStatuses(
                filtersForm.getOrderCode(), filtersForm.getOrderDate(), pageableData, orderStatus);
        final List<OrderStatus> orderList = orderFacade.getOrderStatusList();
        ...
        model.addAttribute(ThirdPartyConstants.SeoRobots.META_ROBOTS, ThirdPartyConstants.SeoRobots.NOINDEX_NOFOLLOW);
        model.addAttribute("orderStatusList", orderList);
        model.addAttribute("solGroupOrderHistoryFiltersForm", filtersForm);
        return getViewForPage(model);
    }
    ...
    public String getProductVariantMatrixForResponsive(@PathVariable("orderCode") final String orderCode,
            @RequestParam(value = "consignmentCode", required = false) final String consignmentCode,
            @RequestParam("productCode") final String productCode,
            @RequestParam("consignmentStatus") final String consignmentStatus,
            final Model model) {
        Map<String, ReadOnlyOrderGridData> readOnlyMultiDMap = null;
        if (StringUtils.isNotEmpty(consignmentCode)) {
            final ConsignmentData consignmentData = consignmentFacade
                    .getConsignmentDetailsForCodeWithoutUser(consignmentCode, orderCode);
            readOnlyMultiDMap = consignmentFacade.getReadOnlyConsignmentGridForProductInOrder(consignmentData);
            model.addAttribute("readOnlyMultiDMap", readOnlyMultiDMap);
            model.addAttribute("consignmentStatus", consignmentStatus);
            return ControllerConstants.Views.Fragments.Checkout.ReadOnlyExpandedConsignmentForm;
        }

        final OrderData orderData = orderFacade.getOrderDetailsForCodeWithoutUser(orderCode);
        readOnlyMultiDMap = orderGridFormFacade
                .getReadOnlyOrderGridForProductInOrder(productCode,
                        Arrays.asList(ProductOption.BASIC, ProductOption.CATEGORIES), orderData);
        model.addAttribute("readOnlyMultiDMap", readOnlyMultiDMap);
        model.addAttribute("isRefill", orderData.getRefill());
        ...
        if (!bindingResult.hasErrors()) {
            if (updatePasswordForm.getNewPassword().equals(updatePasswordForm.getCheckNewPassword())) {
                try {
                    ...
                    bindingResult.rejectValue("newPassword", "changePwd.notValidReason.regex", new Object[] {},
                            "changePwd.notValidReason.regex");
                } catch (final Exception e) {
                    if (LOG.isDebugEnabled()) {
                        LOG.debug(e);
                    }
                    bindingResult.rejectValue("newPassword", "changePwd.notValidReason.generic", new Object[] {},
                            "changePwd.notValidReason.generic");
                }
            } else {
