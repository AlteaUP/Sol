 ...
 */
package com.solgroup.storefront.controllers.misc;
...
public class AddToCartController extends AbstractController
{
	...
	private static final String SHOWN_PRODUCT_COUNT = "solgroupstorefront.storefront.minicart.shownProductCount";
	...
	public final String addQuickOrderToCart(@RequestBody final AddToCartOrderForm form, final Model model)
	{
		...
		final int maxQuickOrderEntries = Config.getInt("solgroupstorefront.quick.order.rows.max", 25);
		...
		form.getCartEntries().stream().limit(Math.min(sizeOfCartEntries, maxQuickOrderEntries)).forEach(cartEntry -> {
			...
			cartEntry.getProduct().setCode(solGroupProductService.getHybrisProductCode(cartEntry.getProduct().getCode(), cmsSiteService.getCurrentSite()));
