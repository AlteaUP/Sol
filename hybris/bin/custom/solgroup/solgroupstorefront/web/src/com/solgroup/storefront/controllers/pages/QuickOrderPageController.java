...
public class QuickOrderPageController extends AbstractPageController
{
	...
	@Resource(name = "solGroupProductFacade")
	private SolGroupProductFacade solGroupProductFacade;
	...
	public String getQuickOrderPage(final Model model) throws CMSItemNotFoundException // NOSONAR
	{
		...
		model.addAttribute("quickOrderMinRows", Integer.valueOf(Config.getInt("solgroupstorefront.quick.order.rows.min", 3)));
		model.addAttribute("quickOrderMaxRows", Integer.valueOf(Config.getInt("solgroupstorefront.quick.order.rows.max", 25)));
	...
	public ProductWrapperData getProductInfo(@RequestParam("code") final String code)
	{
		...
		try
		{
			...
			productData = solGroupProductFacade.getProductForErpCodeAndOptions(code, Arrays.asList(ProductOption.BASIC, ProductOption.PRICE,
