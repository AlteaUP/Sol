 ...
 */
package com.solgroup.storefront.controllers.cms;
...
public class MiniCartComponentController extends AbstractAcceleratorCMSComponentController<MiniCartComponentModel>
{
    ...
    public static final String IS_REFILL = "isRefill";
	...
	protected void fillModel(final HttpServletRequest request, final Model model, final MiniCartComponentModel component)
	{
        ...
        if (BooleanUtils.isTrue(cartData.getRefill())) {
            model.addAttribute(IS_REFILL, Boolean.TRUE);
        } else {
            model.addAttribute(IS_REFILL, Boolean.FALSE);
        }
		...
		if(cartData.getEntries()!=null) {
			...
			model.addAttribute(TOTAL_ITEMS, 0);
		}
