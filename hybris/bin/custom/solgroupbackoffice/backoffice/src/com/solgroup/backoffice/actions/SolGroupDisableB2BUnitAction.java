 ...
 *
 */
public class SolGroupDisableB2BUnitAction implements CockpitAction<B2BUnitModel, Object>
{
	...
	public ActionResult<Object> perform(final ActionContext<B2BUnitModel> ctx)
	{
		...
		if ((data != null) && ((data instanceof B2BUnitModel)))
		{
			...
			// Disable b2bUnit
			solGroupB2BUnitService.disableUnit(b2bUnitModel);
			//Refresh item model
			modelService.refresh(b2bUnitModel);
			// Check if b2bUnit is disabled
			if(BooleanUtils.isFalse(b2bUnitModel.getActive())) {
				// Success message 
				NotificationUtils.notifyUser(ctx.getLabel("com.solgroup.backoffice.action.b2bunitdisable.success"),
						NotificationEvent.Type.SUCCESS);
				return new ActionResult("success", b2bUnitModel);
			}
			else {
				// Error message
				NotificationUtils.notifyUser(ctx.getLabel("com.solgroup.backoffice.action.b2bunitdisable.error"),
	...
	{
	    return ctx.getLabel("com.solgroup.backoffice.action.b2bunitdisable.confirm");
	}
